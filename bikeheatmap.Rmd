...
bike_labels <- data.frame(
  ...
  Full_Name = c(
    ...
    "Contraflow Bike Street",
    "Bike Lane/Shared Lane Hybrid",
    "Separated Bike Lane/Shared Lane Hybrid",
...
roadpal <- colorQuantile("RdYlBu", exposure_list$exposure_metric, n = 5)
...
leaflet() %>%
   ...
   addPolylines(
      ...
      weight = 3,
      opacity = 1,
      label = ~paste0("Street: ", STREET_NAM, " | Crashes: ", crash_count," | Infra: ", ExisFacil),
...
roadpal <- colorQuantile(
  palette = c("#e41a1c","#6250FF", "#00A719"),
  domain = exposure_list$exposure_metric,
  n=17
)
leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  ...
  addPolylines(
    data = bike_network,
    color = ~roadpal(exposure_list$exposure_metric),
    weight = 3,
    opacity = 1,
    label = ~paste0("Street: ", STREET_NAM, " | Crashes: ", crash_count," | Infra: ", ExisFacil),
    group = "Infrastructure"
  ...
  # addLegend(
  #   pal = roadpal, 
  #   values = exposure_list$exposure_metric,
  #   position = "bottomright",
  #   title = "Car Exposure",
  #   labFormat = function(type, cuts, p) {
  #     # This customizes the labels to only show the extremes
  #     return(c("", "","","","","","","","","","","","","","","", ""))
  #   },
  #   opacity = 0.8
  # )
...
bike_network <- bike_network %>%
  left_join(exposure_list, by = c("ExisFacil" = "safety_order"))
...
leaflet() %>%
  ...
  addLegend(
    ...
    values = exposure_list$exposure_metric,
    position = "bottomright",
    title = "Vehicle Exposure",
    labFormat = function(type, cuts, p) {
      # This customizes the labels to only show the extremes
      return(c("Most Exposure", rep("", length(cuts)-2), "Least Exposure"))
    },
    opacity = 1
  )
```
